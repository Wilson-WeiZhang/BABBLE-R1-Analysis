...
clear all;
for permu=1:1:200
    if exist([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/permuentrain/permu',num2str(permu)])==0
        
        mkdir([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/permuentrain/permu',num2str(permu)])
    end
    % warning('off','all')
    ...
    for pt = 1:length(ID) %18 FOR SG, 29 FOR UK
            ...
            for nfamils = 1:length(FamEEGart)
                ...
                for ncond = 1:size(FamEEGart{nfamils,1},1)
                    ...
                    for nphrase = 1:size(FamEEGart{nfamils,1},2)
                        ...
                        possible_ncond= [1, 2, 3];
                        randomized_ncond = possible_ncond(randi(3));
                        possible_phrases = [1, 2, 3];
                        randomized_phrase = possible_phrases(randi(3));
                        sfile = fullfile(wavfiledir, strcat(stim_ids_all{randomized_ncond, randomized_phrase},'.wav'));
                        ...
                        if ~isempty(conddata)
                            ...
                            % calcualte Au_onset_matched length
                            original_length = (samples_first_six_syllables(7) - samples_first_six_syllables(1)) ;
                            
                            % calculate max start point
                            max_start = length(tmpAu3) - original_length + 1;
                            
                            % pick a random start
                            random_start = randi(max_start);
                            
                            % Intercepts a segment of the same length as the original from a randomized starting position
                            Au_onset_matched = tmpAu3(random_start : random_start + original_length - 1);
        ...
        filename = strcat(Fnames,'_permu',num2str(permu),'.mat');
        %% comment by R1 - Commented out to prevent overwriting existing data files
%         save(fullfile([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/permuentrain/permu',num2str(permu),'/'],filename), 'CondMatx', '-v7.3'); % saving Info
