%% ========================================================================
%  STEP 3: Generate Surrogate Data for GPDC Significance Testing
%  ========================================================================
%
%  PURPOSE:
%  Generate 1000 surrogate datasets by randomly shuffling EEG epochs to
%  create chance-level baseline for statistical significance testing of
%  GPDC connectivity values.
%
%  CORRESPONDS TO MANUSCRIPT:
%  - Methods Section 4.3.4 (Generation of surrogate data for significance
%    testing)
%  - "The surrogate dataset was generated by randomly disrupting temporal
%    dependencies between signals, providing a chance-level benchmark"
%
%  SURROGATE GENERATION METHOD:
%  - For each participant and channel:
%    * Infant and adult EEG epochs randomly shuffled in time
%    * Preserves spectral content but disrupts global temporal alignment
%    * 1000 iterations per participant
%  - Same GPDC computation pipeline applied to shuffled data
%  - Group-level surrogate distribution created
%  - 95% CI upper bound = significance threshold (p < .05)
%
%  SIGNIFICANCE TESTING (Methods 4.3.4):
%  - Multiple comparison correction: Benjamini-Hochberg FDR
%  - Connections exceeding 95% CI considered significant
%  - Used to identify real AA, AI, II connections (Fig 3b)
%  - No IA connections significant (as expected for pre-recorded adult)
%
%  OUTPUT:
%  - 1000 surrogate GPDC matrices stored in surrGPDCSET5/ folder
%  - Used for significance testing in fs5_strongpdc.m
%
%  ========================================================================
...
surrPathPrefix = [path,'/infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/surrGPDCSET5/'];
fil=zeros(1000,1);
%
...
for epoc=736:1:740
    if fil(epoc)<=83
        if exist([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/surrPDCSET5/PDC',num2str(epoc),'/'])==0
            mkdir([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/surrPDCSET5/PDC',num2str(epoc),'/'])
        end
        ...
        for loci=1:2
            ...
            for type=3:3
                clearvars -except loci type typelist loclist epoc datauk datasg fil
                ...
                for p = 1:length(PNo)
                    ...
                    if loci==1
                        % filename = strcat(filepath,'\',PNo{p},Loc,'\','P',PNo{p},Loc,'_BABBLE_AR.mat');
                ...
                for p = 1:length(PNo)
                        ...
                        for cond = 1:size(avGPDC{p},2)
                            for phase = 1:size(avGPDC{p},3)
                                if isempty(avGPDC{p}{block,cond,phase})==0 && nansum(nansum(nansum(avGPDC{p}{block,cond,phase})))~=0
                                    ...
                                    GPDC_IA{block,cond,phase} =  tmp(1:length(include),length(include)+1:length(include)*2,:); % AI
                                    GPDC_AI{block,cond,phase} =  tmp(length(include)+1:length(include)*2,1:length(include),:); % IA
                    ...
                    % 分别存储每个变量的频带数据
                    cd([path,'infanteeg/CAM BABBLE EEG DATA/2024/data_matfile/surrPDCSET5/PDC',num2str(epoc),'/'])
                        ...
                        for block=1:size(mytemp1,2)
                            ...
                            for phrase=1:size(mytemp1,3)
                                if isempty(mytemp1{cond,block,phrase})==0
                                    ...
                                    tmpipd2=[tmpipd2,mytemp4{cond,block,phrase}];
                                    tmpipt2=[tmpipt2,mytemp5{cond,block,phrase}];
                                    tmpipa2=[tmpipa2,mytemp6{cond,block,phrase}];
